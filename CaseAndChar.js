...
{ question: "The PHP statement define(\"PI\", 3.14); is used to:", answer: "Define a constant" }
  ...
  questionsAndAnswers.forEach(qa => {
    const questionElem = Array.from(document.querySelectorAll(".qtext")).find(el => el.textContent === qa.question);
    if (questionElem) {
      const answerElem = Array.from(questionElem.parentNode.querySelectorAll("input[type=radio]")).find(input => input.nextElementSibling.textContent === qa.answer);
      if (answerElem) {
        answerElem.click();
        actionPerformed = true;
      }
